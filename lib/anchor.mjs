...
/**
 * 对 /v1/responses 请求体做锚定改写（全程 persona + 持续锚定 + 剥离注入）。
 *
 * - tools：每轮裁剪，目录 = 白名单 ∪ 历史已调用过的工具。
 * - instructions：**全程替换为 minimal persona**（对齐 opencode-anchored：
 *   "之后 system 保持 persona（官方 anchored 全程 minimal system）"）——
 *   完整 system prompt 不再恢复，避免拉回 standard 轨迹；非开局轮追加
 *   一句 hint 让模型按需读 CLAUDE.md（dsh instruction-hint 做法）。
 * - 开局轮（会话首轮 / 用户新消息后第一轮）额外剥离自动注入提醒。
 *
 * 返回 { anchored, body, keptNames }。
 */
export function buildAnchoredResponsesRequest(body, keep = DEFAULT_KEEP, persona = DEFAULT_PERSONA, hint = DEFAULT_HINT) {
